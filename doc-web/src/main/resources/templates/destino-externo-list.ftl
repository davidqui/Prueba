...
<div class="container-fluid">
    ...
    <p>
        ...
        <a href="/admin/destino-externo/create" class="btn btn-success btn-sm">Crear nuevo</a>
    </p>
    ...
    <#else>
                ...
                <#list descriptor.listProperties() as p>
                	...
                	<td nowrap="nowrap">
                            <#if x.activo?? && x.activo == true >
                                <a href=${"/admin/destino-externo/edit?id=" + x.id} class="btn btn-sm btn-warning" title="Modificar">M</a>
                                <a href=${"/admin/destino-externo/delete?id=" + x.id} class="btn btn-sm btn-danger" title="Eliminar" onclick="return confirm('¿Está seguro que desea eliminar el registro?');">E</a>
                            </#if>
                        <#if x.cuando?? >
                            <#if x.cuandoMod?? >
        ...
        <#if totalPages gt 0>
            <@printBar url="/admin/destino-externo" params={"filtro": filtro!"", "all": all?string("true", "false") } metodo="get"/>
        </#if>
    </#if>
