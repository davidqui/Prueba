...
<div class="col-md-12">
    ...
    <form action="/documento/entrada?pin=${instancia.id}" method="POST">
        ...
        <#if documento.soloLectura >
            <div class="form-control">${(documento.destinatario.nombre)!""}</div>
        <#else>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <#else>
                    ...
                    <input class="form-control" id="${spring.status.expression}" name="${spring.status.expression}" value="<#if documento.fechaOficio??>${yyyymmdd.format(documento.fechaOficio)}</#if>"/>
                    <div class="error">
                        <@spring.showErrors "<br>"/>
                    </div>
                </#if>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <#else>
                    ...
                    <input type="text" class="form-control" id="${spring.status.expression}" name="${spring.status.expression}"  value="<#if documento.plazo??>${yyyymmdd.format(documento.plazo)}</#if>" />
                    <div class="error">
                        <@spring.showErrors "<br>"/>
                    </div>
                </#if>
    ...
    <div class="card">
        ...
        <#list documento.adjuntos as adj>
            <div class="card-block">
                <strong>${adj.tipologia.nombre}</strong>
                <a href="/documento/adjunto?doc=${documento.id}&dad=${adj.id}">${adj.original}</a>
            </div>
        </#list>
        <div class="card-block">
            <form action="/documento/adjunto?doc=${documento.id}" method="post" enctype="multipart/form-data">
                <fieldset class="form-group">
                    <label for="destinatario">Tipología</label>
...
<div class="col-md-6 m-y">
    <div class="card">
        <div class="card-header">
            Tipologías de la TRD
        </div>
        <div class="card-block">
            <#list tipologias as tip>
                <#if tip.archivo??>
                    <a href="/ofs/download/${tip.archivo}">${tip.nombre}</a><br/>
                <#else>
                    ${tip.nombre}<br/>
                </#if>
            </#list>
...
<div class="col-md-6 m-y">
    <div class="card">
        ...
        <#list documento.observaciones as obs>
            <div class="card-block">
                <strong>${obs.quien}</strong>, <em>${obs.cuando}</em>
                <p>${obs.texto}</p>
            </div>
        </#list>
        <div class="card-block">
            <form action="/documento/observacion?doc=${documento.id}" method="post" >
                <fieldset class="form-group">
