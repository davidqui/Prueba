...
public class LdapService {
     ...
     * Usuario
     *
     * @param accountName sAMAccountName
    ...
    public Usuario getUsuarioFromLdapByAccountName(String accountName) {
        ...
        List<Usuario> usuariosResult = ldapTemplate.search(query, new AttributesMapper<Usuario>() {
            public Usuario mapFromAttributes(Attributes attrs) throws NamingException {
                 ...
                 */
                String cargoDescripcion = attrs.get(NAME_CARGO) != null ? attrs.get(NAME_CARGO).get().toString() : null;
                //us.setCargo(cargoDescripcion);
                if (attrs.get(NAME_CARGO) != null) {
                    int numRegistros = cargosRepository.findregistrosByNombre(cargoDescripcion);
                    if (numRegistros == 0) {
                        Cargo cargo = new Cargo();
                        cargo.setCarIndLdap(Boolean.TRUE);
                        cargo.setCarNombre(cargoDescripcion);
                        cargosRepository.save(cargo);
                        us.setUsuCargoPrincipalId(cargo);
                    } else {
                        Cargo cargo = cargosRepository.findBycarNombre(cargoDescripcion);
                        us.setUsuCargoPrincipalId(cargo);
                    }
                }
    ...
    /**
     * Obtiene la dependencia del usuario a partir del código de la dependencia
     * en el DA
     *
     * @param codigoDepLdap
     * @return
    ...
    /**
     * Obtiene el grado del usuario a partir del código del grado en el DA, si
     * no encuentra un grado, establece un grado por defecto
     *
     * @param codigoDepLdap
     * @return
     */
