...
public interface DocumentoRepository extends JpaRepository<Documento, String> {
	 ...
	 */
	@Query(nativeQuery = true, value = " SELECT EST.PES_ID, DOC.* " + " FROM DOCUMENTO DOC "
			+ " JOIN S_INSTANCIA_USUARIO HPIN " + " ON DOC.PIN_ID = HPIN.PIN_ID " + " JOIN PROCESO_INSTANCIA PIN "
			+ " ON DOC.PIN_ID = PIN.PIN_ID " + " JOIN PROCESO_ESTADO EST " + " ON EST.PES_ID = PIN.PES_ID "
			+ " JOIN USUARIO USU " + " ON HPIN.USU_ID = USU.USU_ID " + " WHERE USU.USU_LOGIN = ? "
			+ " AND DOC.DOC_RADICADO IS NOT NULL " + " AND EST.PES_FINAL = 1 " + " AND EST.PES_ID NOT IN (83, 101) "
			+ " ORDER BY DOC.CUANDO_MOD DESC ")
	List<Documento> findBandejaEnviados(String name);
