...
public class PlantillaTransferenciaArchivoController extends UtilController {
     ...
     */
    @Autowired
    private PlantillaTransferenciaArchivoService plantillaService;
    ...
    public String presentarFormularioRegistroGET(Principal principal, Model model) {
        final PlantillaTransferenciaArchivo plantilla
                = plantillaService.findPlantillaActiva();
    ...
    public String procesarFormularioRegistroPOST(@RequestParam(value = "file") MultipartFile file,
        ...
        try {
            ...
            model.addAttribute(AppConstants.FLASH_ERROR,
                    "Excepción al almacenar el archivo en el OFS: " + ex.getMessage());
            return presentarFormularioRegistroGET(principal, model);
        }

        model.addAttribute("plantilla", nuevaPlantilla);
        model.addAttribute(AppConstants.FLASH_SUCCESS,
                "Se ha actualizado la plantilla de transferencia de archivo a: "
                + nuevaPlantilla.getNombreArchivo() + ".");
