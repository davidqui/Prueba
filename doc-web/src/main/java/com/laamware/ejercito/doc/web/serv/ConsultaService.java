    ...
    */
    private StringBuilder retornaConsultaPrincipal() {
        ...
        return new StringBuilder(""
                ...
                + "LEFT JOIN (SELECT CONNECT_BY_ROOT DEP_ID AS DEP_ORI_ID, CONNECT_BY_ROOT DEP_SIGLA AS DEP_ORI_NOMBRE, DEP_ID FROM DEPENDENCIA START WITH DEP_PADRE IS NULL CONNECT BY DEP_PADRE = PRIOR DEP_ID ) DEP_ORIGEN ON (DEP_ORIGEN.DEP_ID = USU_ELABORA.DEP_ID )\n"
                + "LEFT JOIN (SELECT CONNECT_BY_ROOT DEP_ID AS DEP_DES_ID, CONNECT_BY_ROOT DEP_SIGLA AS DEP_DES_NOMBRE, DEP_ID FROM DEPENDENCIA START WITH DEP_PADRE IS NULL CONNECT BY DEP_PADRE = PRIOR DEP_ID ) DEP_DESTINO ON (DEP_DESTINO.DEP_ID = DOC.DEP_ID_DES)\n"
                + "WHERE 1 = 1 \n");
    }
}
