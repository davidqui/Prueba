...
public class Instancia extends AuditModifySupport {
    ...
    public boolean forward(Integer tid) {
        for (Transicion transicion : transiciones()) {
            if (Objects.equals(transicion.getId(), tid)) {
    ...
    public List<Transicion> transiciones() {
        List<Transicion> transicionesCumple = new ArrayList<>();
         ...
         */
        boolean procesoExterno = Objects.equals(getProceso()
                .getId(), Proceso.ID_TIPO_PROCESO_GENERAR_DOCUMENTOS_PARA_ENTES_EXTERNOS_O_PERSONAS);
        boolean estadoEnviado = Objects.equals(getEstado().getId(), Estado.ENVIADO);
            ...
            if (this.getAsignado().getLogin().equals(username)) {
                ...
                for (Transicion transicion : transiciones) {
                    ...
                    if (cumple) {
                         ...
                         */
                        if (!(procesoExterno && estadoEnviado && transicion.getNombre().trim().equalsIgnoreCase(nombreTransicionDarRespuesta))) {
