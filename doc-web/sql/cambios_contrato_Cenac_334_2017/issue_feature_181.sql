...
ALTER TABLE PAR_NOMBRE_EXPEDIENTE ADD CONSTRAINT FKPAR_NOMBRE134796 FOREIGN KEY (USU_CREACION) REFERENCES USUARIO (USU_ID);
ALTER TABLE PAR_NOMBRE_EXPEDIENTE ADD CONSTRAINT FKPAR_NOMBRE698783 FOREIGN KEY (USU_MODIFICACION) REFERENCES USUARIO (USU_ID);
...
CREATE TABLE EXPEDIENTE_ESTADO (
  ...
  EST_NOMBRE number(10), 
  PRIMARY KEY (EXP_EST_ID));
COMMENT ON COLUMN EXPEDIENTE_ESTADO.EXP_EST_ID IS 'Identificador unico de la tabla';
COMMENT ON COLUMN EXPEDIENTE_ESTADO.EST_NOMBRE IS 'Nombre del estado de la transición';
